...
function extendUrlWithSSRParams(url, tokenInHeader) {
	//ie does not support functions with default value
	...
	if (typeof URLSearchParams === 'undefined') {
		window.URLSearchParams = window.frameElement ? window.parent.URLSearchParams : window.opener.URLSearchParams;
	}
	//Additional parameters, e.g. OAuth token renewal
	...
	urlSearchParams.set('LOCALE', i18nSessionContext.getLocale());
	urlSearchParams.set('TIMEZONE_NAME', i18nSessionContext.getTimeZone());
	...
	if (typeof (window.izendaPageId$) !== 'undefined') {
		urlSearchParams.set('izpid', window.izendaPageId$);
	}
	// in Post requests token pass through header, in Get via url
	if (tokenInHeader) {
		const authorizationHeaderName = aras.OAuthClient.authorizationHeaderName;
		const authorizationHeader = aras.OAuthClient.getAuthorizationHeader();
		const authorizationHeaderValue = authorizationHeader[authorizationHeaderName];
		return {
			url: newUrl,
	...
	urlSearchParams.set('access_token', aras.OAuthClient.getToken());
	return {
		url: newUrl,
		params: urlSearchParams.toString()
	};
...
function AjaxRequest(url, parameters, callbackSuccess, callbackError, id, dataToKeep, async) {
	...
	const newUrl = urlWithParams.url + '?' + urlWithParams.params;

	// run query:
	thisRequestObject.open('POST', newUrl, async);
	thisRequestObject.setRequestHeader(urlWithParams.authorizationHeaderName, urlWithParams.authorizationHeaderValue);
