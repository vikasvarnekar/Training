...
function SC_GetSelectedFormats(id){
	...
	for (var i = 0; i < body.rows.length; i++) {
		...
		// add selectedIndex check
		if(!formatSel || !formatSel.options || formatSel.selectedIndex < 0) continue;
...
function SC_GetFieldsList(id, columnName, functionName) {
	...
	for (var i = 0; i < body.rows.length; i++) {
		...
		if (columnSel != null) {
			...
			// add selectedIndex check
			if (columnSelValue !== '...' && columnSel.selectedIndex >= 0) {
				var field = {};
				...
				// add selectedIndex check
				if (funcSelect != null && funcSelect.selectedIndex >= 0) {
...
function SC_OnExtraColumnChangedHandler(e, el, columnName) {
	...
	if (row != null) {
		...
		if (cName == 'ExtraColumn') {
			...
			// add selectedIndex check
			if (columnSel && columnSel.selectedIndex >= 0 && descriptionEdit) {
...
function SC_OnColumnChangedHandler(e, el) {
	...
	if (row != null && row.parentNode != null) {
		...
		try {
			...
			// add selectedIndex check
			var isSameValue = oldValue != null && columnSel.selectedIndex >= 0 && columnSel.options[columnSel.selectedIndex].value == oldValue;
			...
			// add selectedIndex check
			if (oldValue != null && columnSel.selectedIndex >= 0 && columnSel.options[columnSel.selectedIndex].value != oldValue) {
...
function SC_ResetRowToDefault(context) {
	...
	if (row && columnSel) {
		...
		while (!mustGroupOrFunction && i < count) {
			...
			//add selectedIndex check
			var isScalar = (funcTemp == null || funcTemp.selectedIndex < 0) ? null : funcTemp.options[funcTemp.selectedIndex].getAttribute("isScalar");
		...
		for (var i = 0; i < rowCount; i++) {
			...
			if (columnSelect != null) {
				if (columnSelect.value != '...') {
					...
					// add selectedIndex check
					var isScalar = funcSelect.selectedIndex < 0 ? null : funcSelect.options[funcSelect.selectedIndex].getAttribute("isScalar");
