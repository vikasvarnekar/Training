...
function GC_OnColumnChangedHandler(e) {
	...
	try {
		...
		// add selectedIndex check and columnSel
		if (!columnSel || columnSel.disabled || columnSel.selectedIndex < 0)
			return;
	...
	for (var i = 1; i < body.rows.length; i++)
	{
		...
		if (i > disableAfterIndex)
		{
			...
			if (selFunc != null && !selField.disabled && selField.selectedIndex >= 0 && selField.options[selField.selectedIndex].value.indexOf(calcFieldPrefix) == -1)
