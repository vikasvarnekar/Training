...
function FP_ShowFieldProperties(field, dialog) {
	...
	var propDescription = document.getElementById('propDescription');
	propDescription.value = field.FriendlyName;
...
function beautifyDialogControls(dialog) {
		dialog.dialog("option", "resizable", false);;
		dialog.dialog("option", "width", "460");
		dialog.dialog("option", "dialogClass", "dialog_properties");

		function assignBtnClass(button, cls) {
			var btnNode = jq$("div.dialog_properties button.iz-ui-button.iz-ui-widget:" + button);
			btnNode.removeClass();
			btnNode.addClass(cls);
		}

		function handleSelect(selectId) {
			var selectControl = jq$(selectId);
			if (selectControl) {
				function setValue(self) {
					var displaySpanId = "#span_" + selectId.replace("#", "");
					var displaySpan = self.parentNode.querySelector(displaySpanId);
					var selectedOptions = self.querySelectorAll("option[value='" + self.value + "']");
					if (selectedOptions && selectedOptions.length > 0) {
						displaySpan.textContent = selectedOptions[0].label;
					}
				}

				setValue(selectControl[0]);
		...
		handleSelect("#propFormats");
		handleSelect("#propFilterOperators");
		assignBtnClass("last", "btn cancel_button");
		assignBtnClass("first", "btn");
}
