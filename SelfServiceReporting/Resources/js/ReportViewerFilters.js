...
function GenerateNewFilterDropDown() {
	var result = '<div class="sys_f_div_select" >' +
		'<select id="newFilterFieldDropDown" class="newFilterFieldDropDown sys_f_select" onchange="AddNewFilterField();">';
	...
	var filterArray = [], filterName = [];
	for (var dsCnt = 0; dsCnt < dataSources.length; dsCnt++) {
				var columnsFilter = fieldsList.filter(function (obj) {
					return (obj.DbName.indexOf(dataSources[dsCnt].DbName) === 0 && !obj.FilterHidden && existingParamFilters.indexOf(obj.DbName) === -1);
		}).map(function (obj) {
			return '<option value="' + obj.DbName + '" data-alias="' + dataSources[dsCnt].JoinAlias + '">' + (obj.FilterFriendlyName ? obj.FilterFriendlyName : obj.FriendlyName) + '</option>';
		}).join("");
		if (columnsFilter) {
			filterName.push(dataSources[dsCnt].FriendlyName);
			filterArray.push(columnsFilter);
		}
	}
	if (filterArray.length > 1) {
		result += filterName.map(function (str, index) {
			return '<optgroup label="' + str + '">' + filterArray[index] + '</optgroup>';
		}).join("");
	} else if (filterArray.length === 0) {
		return "";
	}
	result += (filterArray.length == 1 ? filterArray[0] : "") + '</select><span class="sys_f_span_select">...</span></div>';
	return result;
}
...
function CheckShowAddFilterControls() {
	if (nrvConfig == null || (nrvConfig.AllowNewFiltersInReportViewer && (nrvConfig.ReportIsLocked == null || nrvConfig.ReportIsLocked == false))) {
		...
		addFilterHtml += '<input tabindex="0" class="filter_criteria disaled" type="text" readonly="" /></div></div>';
		...
		Innovator_ShowHideAddFilter();
	}
}

function Innovator_ShowHideAddFilter() {
	var newFilterColumnSelDiv = document.getElementById('newFilterColumnSel');
	var fuidNewFilterDiv = document.getElementById('fuidNewFilter');
	var isExpanded = fuidNewFilterDiv.getAttribute('expanded') == 'true';
	if (isExpanded) {
		...
		fuidNewFilterDiv.style.width = '250px';
		newFilterColumnSelDiv.style.display = '';
		fuidNewFilterDiv.setAttribute('expanded', 'true');
		newFilterColumnSelDiv.innerHTML += '<div class="filterRemoveButton disabled"></div><div class="filterPropertiesButton disabled"></div>';
	}
}
...
function GenerateFilterControl(index, cType, value, values, existingLabels, existingValues, isLastFilter) {
	...
	switch (cType) {
		case 1:
			if (value === '...') value = '';
			result = '<input class="filter_criteria" type="text" id="ndbfc' + index + '" value="' + value.replaceAll('"', "&quot;") + '" ' + onChangeCmd + ' />';
			break;
		case 2:
		...
		case 3:
			...
			var id = 'ndbfc' + index;
			var selectedLabel = ''
			result += '<div class="sys_f_div_select" id="div_' + id + '">' +
				'<select class="sys_f_select" style="width:100%;" id="' + id + '" ' + onChangeCmd + '>';
			for (var cnt3 = 0; cnt3 < existingValues.length; cnt3++) {
				var selected3 = '';
				if (existingValues[cnt3] != null && existingValues[cnt3].LinelessEquals(value)) {
					selected3 = 'selected="selected"';
					selectedLabel = existingLabels[cnt3];
				}
			...
			if (groupOpened)
				result += '</optgroup>';
			result += '</select><span id="span_' + id + '" class="sys_f_span_select">' + selectedLabel + '</span></div>';
			break;
		case 4:
			var id = 'ndbfc' + index;
			var selectedLabel = ''
