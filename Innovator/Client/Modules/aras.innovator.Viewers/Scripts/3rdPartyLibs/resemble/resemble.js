...
(function (_this) {
	...
	_this.resemble = function (fileData) {
		...
		function loop(x, y, callback, progressCallback) {
			var i, j, currentPercentage = 25;
			for (i = 0; i < x; i++) {
				for (j = 0; j < y; j++) {
					var tempPercentage = Math.round((i * y + j) / (x * y) * 100);
					if (tempPercentage > currentPercentage + 5) {
		...
		function analyseImages(img1, img2, width, height, progressCallback) {
		...
		function compare(one, two, progressCallback) {

			progressCallback(5);
			var prgrsCallback = progressCallback;
			function onceWeHaveBoth() {
				...
				if (images.length === 2) {
					...
					analyseImages(normalise(images[0], width, height), normalise(images[1], width, height), width, height, prgrsCallback);
		...
		function getCompareApi(param) {
			...
			var self = {
				//value mast be between 0 and 255
				setTolerance: function (toleranceValue) {
					tolerance.red = toleranceValue;
					tolerance.green = toleranceValue;
					tolerance.blue = toleranceValue;
					tolerance.alpha = toleranceValue;
				...
				onComplete: function (callback, progressCallback) {
					updateCallbackArray.push(callback);
					var wrapper = function () {
						compare(fileData, secondFileData, progressCallback);
					};
	...
	_this.resemble.outputSettings = function (options) {
		...
		if (options.errorColor) {
			for (key in options.errorColor) {
				errorPixelColor[key] = options.errorColor[key] === undf ? errorPixelColor[key] : options.errorColor[key];
			}
		}
		...
		if (options.largeImageThreshold !== undf) {
