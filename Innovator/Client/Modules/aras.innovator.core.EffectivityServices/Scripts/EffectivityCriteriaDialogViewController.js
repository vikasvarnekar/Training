(function (window) {
	...
	const CONTAINER_MARGIN_WIDTH = 18;
	...
	function EffectivityCriteriaDialogViewController(dialogArguments) {
	...
	EffectivityCriteriaDialogViewController.prototype = {
		...
		set _scopeItemContext(itemNode) {
			this._currentScopeItem = {
				...
				builderMethod:
					this.aras.getItemPropertyAttribute(
						itemNode,
						'builder_method',
						'keyed_name'
					) || ''
			...
			this._toggleElementDisabledState(
		...
		_init: function (
			...
			clearButtonConnectId
		) {
			//init soap module for itemProperty component
			ArasModules.soap(null, {
				async: true,
				method: 'ApplyItem',
				url: this.aras.getServerURL(),
				headers: this.aras.getHttpHeadersForSoapMessage('ApplyItem')
			});
			...
			return this._loadGrid(gridConnectId)
				.then(
					function () {
				...
				.then(
					function () {
						return this._updateGridCellsWithValues(
							GRID_COLUMN_NAMES.value,
							this._presetScope ? this._presetScope.variables : null,
							'id',
							'value'
						);
					}.bind(this)
		...
		_defineCommonGridCellEditorMetadata: function () {
			const failedStringValidationMessage = this.aras.getResource(
				...
				'effectivity_criteria_dialog.grid.cell_editor.invalid_string_validation_message'
			);
			const failedIntegerValidationMessage = this.aras.getResource(
				this._effsModuleSolutionBasedRelativePath,
				'effectivity_criteria_dialog.grid.cell_editor.invalid_integer_validation_message'
			);
			const failedListValueValidationMessage = this.aras.getResource(
				this._effsModuleSolutionBasedRelativePath,
				'effectivity_criteria_dialog.grid.cell_editor.invalid_list_value_validation_message'
			);
			...
			this._commonGridCellEditorMetadata = {
				...
				integer: {
					...
					checkValidity: function (value) {
						...
						if (value) {
							...
							//value in Javascript
							isValidValue =
								Number.isInteger(intValueOrNaN) &&
								intValueOrNaN.toString() === value &&
								intValueOrNaN >= INT32_MIN_VALUE &&
								intValueOrNaN <= INT32_MAX_VALUE;
						}
				...
				list: {
					...
					willValidate: function () {
						const filterList = this.grid.view.activeCell.querySelector(
							'aras-filter-list'
						);
						...
						return foundOption
							? Promise.resolve(foundOption.value)
							: Promise.reject(failedListValueValidationMessage);
					}.bind(this)
				}
			};
		},
		...
		_loadGrid: function (gridConnectId) {
			...
			const columnWidth = Math.floor(
				(window.innerWidth - CONTAINER_MARGIN_WIDTH * 2) / 2
			);
			const columns = [
				{
					...
					label: this.aras.getResource(
						this._effsModuleSolutionBasedRelativePath,
						'effectivity_criteria_dialog.grid.variable_column_label'
					),
					width: columnWidth,
					settings: {
						editable: false
				...
				},
				{
					...
					label: this.aras.getResource(
						this._effsModuleSolutionBasedRelativePath,
						'effectivity_criteria_dialog.grid.value_column_label'
					),
					width: columnWidth
				}
			];
			...
			switch (variable.datatype) {
				...
				default:
					...
					metadata = Object.assign(
						{ options: options },
						this._commonGridCellEditorMetadata.list
					);
					break;
			}
		...
		_loadToolbar: function (toolbarConnectId) {
			...
			Toolbar.extendFormatters(effs_cuiFormatters);
			document.getElementById(toolbarConnectId).appendChild(toolbar);

			const itemFieldHandlers = this._getScopeFieldToolbarItemMetadata();

			//'itemFieldHandlers' object will be passed as metadata to the 'effs_item_field' toolbar formatter through
			//a command bar item initialization handler
			return window.cuiToolbar(
				toolbar,
				'effs_effectivityCriteriaDialogToolbar',
				{ metadata: itemFieldHandlers }
			);
		},
		...
		_getScopeFieldToolbarItemMetadata: function () {
			...
			const showSearchDialog = function (itemPropertyElement) {
				topWnd.ArasModules.MaximazableDialog.show('iframe', {
					...
					if (currentRetainedScopeItemId !== result.itemID) {
						...
						itemPropertyElement.state.refs.input.dispatchEvent(
			...
			return {
				onChangeHandler: function () {
					...
					const currentRetainedScopeItemKeyedName =
						self._scopeItemContext.keyedName;

					if (
						selectedScopeItemKeyedName === currentRetainedScopeItemKeyedName
					) {
						return;
					}

					const clearCurrentRetainedScopeItem =
						selectedScopeItemKeyedName === '';
					const scopeItemNode = clearCurrentRetainedScopeItem
						? null
					...
					if (clearCurrentRetainedScopeItem || scopeItemNode) {
						if (currentRetainedScopeItemKeyedName) {
							...
							topWnd.ArasModules.Dialog.confirm(message).then(function (
								result
							) {
								...
								if (isScopeItemChangingConfirmed) {
									...
									itemPropertyElementState.value = currentRetainedScopeItemKeyedName;
								}
						...
						topWnd.ArasModules.Dialog.confirm(message).then(function (result) {
							...
							if (giveAnotherAttemptToEnterItemName) {
								...
								itemPropertyElement.setState({
									value: currentRetainedScopeItemKeyedName
								});
							}
						});
					}
				...
				onAddElementNodeHandler: function (itemPropertyElement) {
					self._scopeItemField = itemPropertyElement;
					self._scopeItemContext = self._presetScope
						? self.aras.getItemById(
								itemPropertyElement.state.itemType,
					...
					itemPropertyElement.setState({
						value: self._scopeItemContext.keyedName
					});
				}
			};
		},
		...
		_getScopeObject: function () {
			...
			const targetScope = this.aras.newIOMItem(
				'Method',
				this._scopeItemContext.builderMethod
			);
		...
		_parseScopeItemToObject: function (scopeItem) {
			...
			for (let varIndex = 0; varIndex < variableCount; varIndex++) {
				...
				const namedConstantItems = variableItem.getRelationships(
					'NamedConstant'
				);
		...
		_updateGridCellsWithValues: function (
			gridColumnName,
			valueInstanceArray,
			rowIdPropertyNameInValueInstance,
			valuePropertyNameInValueInstance
		) {
			if (!valueInstanceArray) {
				return;
			}
			...
			Array.prototype.forEach.call(
				...
				function (valueInstance) {
					this.grid.setCellValue(
						valueInstance[rowIdPropertyNameInValueInstance],
						gridColumnName,
						valueInstance[valuePropertyNameInValueInstance]
					);
				}.bind(this)
			);
		},
		...
		_setupApplyButton: function (applyButtonConnectId) {
			...
			this._setupButton(
				...
				'effectivity_criteria_dialog.apply_button_label',
				this._applyButtonOnClickHandler.bind(this)
			);
		},
		...
		_setupClearButton: function (clearButtonConnectId) {
			...
			this._setupButton(
				this._clearButtonElement,
				'effectivity_criteria_dialog.clear_button_label',
				this._clearButtonOnClickHandler.bind(this)
			);
		},

		_setupButton: function (buttonElement, textContentResource, eventListener) {
			buttonElement.firstElementChild.textContent = this.aras.getResource(
				this._effsModuleSolutionBasedRelativePath,
				textContentResource
			);
		...
		_applyButtonOnClickHandler: function () {
			const isModelStateValid = this.grid.settings.focusedCell
				? this.grid.settings.focusedCell.valid
				: true;
			...
			if (!isModelStateValid) {
				return this.aras.AlertError(
					this.aras.getResource(
						this._effsModuleSolutionBasedRelativePath,
						'effectivity_criteria_dialog.apply_button_validation_message'
					)
				);
			}
		...
		_clearButtonOnClickHandler: function () {
			this._scopeItemContext = null;
			this._scopeItemField.setState({ value: null });
		}
	};

	Object.defineProperty(
		EffectivityCriteriaDialogViewController,
		'createInstance',
		{
			value: function (options) {
				const criteriaDialogViewControllerInstance = new EffectivityCriteriaDialogViewController(
					options.dialogArguments
				);

				return criteriaDialogViewControllerInstance
					._init(
						options.toolbarConnectId,
						options.gridConnectId,
						options.applyButtonConnectId,
						options.clearButtonConnectId
					)
					.then(function () {
						return criteriaDialogViewControllerInstance;
					});
