define([
	...
	var DISABLED = 0,
		...
		_inRange = function (type, value, start, end, halfClose) {
			if (type !== 'cell') {
				...
				return (
					_inRange('col', value, start, end, halfClose) &&
					_inRange('row', value, start, end, halfClose)
				);
			}
		},
				...
				if (v1 && v2) {
					switch (type) {
						...
						case 'row':
							return (
								v1[type] == v2[type] &&
								typeof v1[type] == 'number' &&
								!(_theOther[type] in v1) &&
								!(_theOther[type] in v2)
							);
						case 'cell':
							return (
								v1.cell == v2.cell &&
								v1.row == v2.row &&
								typeof v1.cell == 'number' &&
		...
		_createItem = function (type, rowIndex, colIndex, cellIndex) {
			switch (type) {
				case 'col':
					return {
	...
	var Selector = declare(
		'Aras.Client.Controls.Experimental.Selector',
		_Plugin,
		{
			// summary:
		...
		*/
			...
			toggleSelect: function (type, rowIndex, colIndex, cellIndex) {
				this._startSelect(
					type,
					_createItem(type, rowIndex, colIndex, cellIndex),
					this._config[type] === MULTI,
					false,
					false,
					!this.isSelected(type, rowIndex, colIndex)
				);
				this._endSelect(type);
			},
			select: function (type, rowIndex, colIndex, cellIndex) {
				// summary:
				...
				if (!this.isSelected(type, rowIndex, colIndex)) {
					this.toggleSelect(type, rowIndex, colIndex, cellIndex);
				}
			},
			deselect: function (type, rowIndex, colIndex, cellIndex) {
				if (this.isSelected(type, rowIndex, colIndex)) {
					this.toggleSelect(type, rowIndex, colIndex, cellIndex);
				}
			},
			selectRange: function (type, start, end, toSelect) {
				...
				var startPoint =
						type == 'cell'
							? _createItem(type, start.row, start.col)
							: _createItem(type, start),
					endPoint =
						type == 'cell'
							? _createItem(type, end.row, end.col)
							: _createItem(type, end);
				this._startSelect(type, startPoint, false, false, false, toSelect);
				this._highlight(
					type,
					endPoint,
					toSelect === undefined ? true : toSelect
				);
				this._endSelect(type);
			...
			isSelecting: function (type) {
				...
				if (typeof type == 'undefined') {
					return (
						this._selecting.col || this._selecting.row || this._selecting.cell
					);
				}
				...
				switch (type) {
					...
					case 'row':
						...
						if (includeExceptions) {
							arrayResult = this._selected[type];
						} else {
							arrayResult = array.filter(this._selected[type], function (item) {
								return item.except.length === 0;
							});
						}
						return array.map(arrayResult, function (item) {
				...
				if (rowSelector instanceof _RowSelector) {
					rowSelector.doStyleRowNode = function (inRowIndex, inRowNode) {
						...
						html.toggleClass(
							inRowNode,
							'dojoxGridRowbarOver',
							g.rows.isOver(inRowIndex)
						);
						html.toggleClass(
							inRowNode,
							'dojoxGridRowbarSelected',
							!!g.selection.isSelected(inRowIndex)
						);
					};
				}
			...
			_initEvents: function () {
				...
				var g = this.grid,
					...
					starter = function (type, e) {
						...
						if (_this.selectEnabled() && _this._config[type] && e.button != 2) {
							if (
								_this._keyboardSelect.col ||
								_this._keyboardSelect.row ||
								_this._keyboardSelect.cell
							) {
							...
							var colIndex = parseInt(e.cellNode.getAttribute('col'), 10);
							var cellIndex = parseInt(e.cellNode.getAttribute('idx'), 10);
							var target = _createItem(type, rowIndex, colIndex, cellIndex);
				...
				this.connect(g, 'onCellMouseOver', function (e) {
					if (
						_this._curType != 'row' &&
						_this._selecting[_this._curType] &&
						_this._config[_this._curType] == MULTI
					) {
						_this._highlight(
							'col',
							_createItem('col', e.cell.index),
							_this._toSelect
						);
						if (!_this._keyboardSelect.cell) {
							_this._highlight(
								'cell',
								_createItem('cell', e.rowIndex, e.cell.index),
								_this._toSelect
							);
						}
					}
				...
				this.connect(g, 'onEndAutoScroll', function (
					isVertical,
					isForward,
					view,
					target
					...
					if (isVertical && (selectCell || _this._selecting.row)) {
						...
						_this._highlight(
							type,
							_createItem(type, current.row + dir, current.col),
							_this._toSelect
						);
						...
						_this._highlight(
							type,
							_createItem(type, current.row, target),
							_this._toSelect
						);
					}
				});
				...
				try {
					...
					if (type === 'cell') {
						...
						array.forEach(pointSet, function (item) {
							...
							for (
								var r = mapping.from.min.row, tr = mapping.to.min.row;
								r <= mapping.from.max.row;
								++r, ++tr
							) {
								for (
									var c = mapping.from.min.col, tc = mapping.to.min.col;
									c <= mapping.from.max.col;
									++c, ++tc
								) {
			...
			_initAreas: function () {
						...
						if (evt.shiftKey && ks[type]) {
							if (ks[type] === keyboardSelectReady) {
								if (type === 'cell') {
									...
									if (
										f.cell != g.layout.cells[item.col + colStep] ||
										f.rowIndex != item.row + rowStep
									) {
										ks[type] = 0;
										return;
									}
						...
						if (
							...
							switch (evt.keyCode) {
								...
								case keys.SHIFT:
									...
									if (
										_this._config[type] == MULTI &&
										_this._isValid(type, _this._lastAnchorPoint[type], g)
									) {
					...
					f.addArea({
						...
						onFocus: function (evt, step) {
							...
							if (view instanceof _RowSelector) {
								var rowBarNode = view.getCellNode(
									_this._lastFocusedRowBarIdx,
									0
								);
								if (rowBarNode) {
									...
									if (next > lastPageRow) {
										g.setScrollTop(
											g.scrollTop +
												...
												sc.findScrollTop(_this._lastFocusedRowBarIdx)
										);
									}
				...
				f.addArea({
					name: 'cellselect',
					onMove: lang.partial(onmove, 'cell', function (
						type,
						rowStep,
						colStep,
						evt
				...
				f.addArea({
					name: 'colselect',
					onMove: lang.partial(onmove, 'col', function (
						type,
						rowStep,
						colStep,
						evt
				...
				f.addArea({
					name: 'rowselect',
					onMove: lang.partial(onmove, 'row', function (
						type,
						rowStep,
						colStep,
						evt
			...
			_startSelect: function (
				...
				if (!extending || (!isSelected && this._config[type] == SINGLE)) {
					...
					if (
						!this.noClear ||
						(type === 'row' && this._config[type] == SINGLE)
					) {
					...
					if (this._usingKeyboard && !toHighlight) {
						...
						if (last) {
							...
							if (
								!toSelect &&
								!sltd &&
								this._isInLastRange(this._lastType, target, true)
							) {
								return true;
							}
						}
				...
				switch (type) {
					case 'cell':
						...
						if (!c.hidden && !c.notselectable) {
							var cellNode = this._getView().getCellNode(
								target.row,
								target.cell
							);
							this._highlightNode(cellNode, toHL);
						}
						break;
					case 'col':
						toHL = this._calcToHighlight(type, target, toHighlight, toSelect);
						this._highlightHeader(target.col, toHL);
						query("td[idx='" + target.col + "']", g.domNode).forEach(function (
							cellNode
						) {
							var rowNode = cells[target.col].view.content.findRowTarget(
								cellNode
							);
							if (rowNode) {
				...
				if (this._selecting[type] && target !== null) {
					...
					switch (type) {
						...
						case 'cell':
							if (current !== null) {
								if (
									_inRange('row', target, start, current, true) ||
			...
			_focusPoint: function (type, point) {
				...
				if (!this._isStartFocus) {
					...
					if (type == 'col') {
						...
						f.setFocusIndex(current.row, current.cell);
					}
				}
			},
			...
			_add: function (type, items) {
				...
				if (type == 'cell') {
					...
					items = array.filter(items, function (item) {
						return (
							array.indexOf(colMakedup, item) < 0 &&
							array.indexOf(rowMakedup, item) < 0 &&
							!cells[item.col].hidden &&
							!cells[item.col].notselectable
						);
					});
				} else {
					if (type == 'col') {
					...
					this._selected[type] = array.filter(this._selected[type], function (
						v
					) {
			...
			_isInLastRange: function (type, item, isSelected) {
				...
				var start = this[
						isSelected ? '_lastSelectedAnchorPoint' : '_lastAnchorPoint'
					][type],
					end = this[isSelected ? '_lastSelectedEndPoint' : '_lastEndPoint'][
						type
					];
				if (!item || !start || !end) {
					return false;
				...
				try {
					...
					switch (type) {
						case 'col':
							return (
								index >= 0 &&
								index < g.layout.cells.length &&
								lang.isArray(item.except) &&
								(allowNotSelectable || !g.layout.cells[index].notselectable)
							);
						case 'row':
							return (
								index >= 0 && index < g.rowCount && lang.isArray(item.except)
							);
						case 'cell':
							return (
								item.col >= 0 &&
